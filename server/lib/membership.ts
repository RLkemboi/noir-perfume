...
export const TIER_POLICIES: Record<UserTier, TierPolicy> = {
  ...
  Bronze: {
    ...
    promotionMinLifetimeSpend: 5000,
  ...
  Silver: {
    ...
    promotionMinLifetimeSpend: 25000,
    allowedPaymentMethods: ["Card", "Mpesa", "PayOnDelivery"],
  ...
  Gold: {
    ...
    allowOverdraft: true,
    maxOutstandingBalance: 25000,
    promotionMinCompletedOrders: 12,
    promotionMinLifetimeSpend: 100000,
    allowedPaymentMethods: ["Card", "Mpesa", "PayOnDelivery"],
  ...
  Platinum: {
    ...
    allowFullCredit: true,
    allowOverdraft: true,
    maxOutstandingBalance: 75000,
    promotionMinCompletedOrders: 25,
    promotionMinLifetimeSpend: 300000,
    allowedPaymentMethods: ["Card", "Mpesa", "PayOnDelivery"],
  ...
  Black: {
    ...
    promotionMinLifetimeSpend: 750000,
    allowedPaymentMethods: ["Card", "Mpesa", "PayOnDelivery"],
...
export function evaluatePaymentPolicy(input: PaymentPolicyEvaluationInput): PaymentPolicyEvaluation {
  ...
  if (policy.maxOutstandingBalance != null && projectedOutstandingBalance > policy.maxOutstandingBalance) {
    return {
      allowed: false,
      reason: `${tier} maximum outstanding balance is KES ${Math.round(policy.maxOutstandingBalance).toLocaleString()}.`,
      policy,
      upfrontAmount,
      ledgerCharge,
